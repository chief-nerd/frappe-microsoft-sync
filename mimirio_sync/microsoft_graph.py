...
class MicrosoftGraphClient:
    ...
    def get_sync_settings(self):
        settings = frappe.get_all("MS Sync Settings", filters={"user": self.user}, fields=["*"])
        if not settings:
            doc = frappe.get_doc({
                "doctype": "MS Sync Settings",
                "user": self.user
            })
    ...
    def get_access_token(self):
        ...
        refresh_token = get_decrypted_password("User", self.user, "microsoft_refresh_token", raise_exception=False)
        if not refresh_token:
            # Try to get from MS Sync Settings if it's there
            refresh_token = get_decrypted_password("MS Sync Settings", self.settings.name, "refresh_token", raise_exception=False)
        
        if not refresh_token:
            frappe.log_error(title="MS Sync Error", message=f"No Microsoft refresh token found for user {self.user}")
            return None

        # Refresh the token
        provider_name = "Office 365" if frappe.db.exists("Social Login Key", "Office 365") else "Microsoft"
        social_login_key = frappe.get_doc("Social Login Key", provider_name)
        
        data = {
            "client_id": social_login_key.client_id,
            "client_secret": get_decrypted_password("Social Login Key", provider_name, "client_secret"),
        ...
        if response.status_code != 200:
            frappe.log_error(title="MS Sync Error", message=f"Failed to refresh Microsoft token for {self.user}: {response.text}")
            return None
        ...
        if new_refresh_token:
            set_encrypted_password("MS Sync Settings", self.settings.name, new_refresh_token, "refresh_token")
    ...
    def get_or_create_todo_list(self):
        if self.settings.microsoft_list_id:
            ...
            res = self.request("GET", f"/me/todo/lists/{self.settings.microsoft_list_id}")
    ...
    def sync_todo(self, todo_doc):
        ...
        if todo_doc.microsoft_todo_id:
            # Update
            res = self.request("PATCH", f"/me/todo/lists/{list_id}/tasks/{todo_doc.microsoft_todo_id}", data=task_data)
            if res and res.status_code == 404:
                # Task deleted in MS, recreate it
                res = self.request("POST", f"/me/todo/lists/{list_id}/tasks", data=task_data)
        else:
            # Create
            res = self.request("POST", f"/me/todo/lists/{list_id}/tasks", data=task_data)
        ...
        if res and res.status_code in [200, 201]:
            ...
            if ms_todo_id != todo_doc.microsoft_todo_id:
                frappe.db.set_value("ToDo", todo_doc.name, "microsoft_todo_id", ms_todo_id, update_modified=False)
